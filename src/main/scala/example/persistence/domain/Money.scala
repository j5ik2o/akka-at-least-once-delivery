...
class Money(private val amount: BigDecimal, private val currency: Currency) extends Ordered[Money] with Serializable {
  ...
  /** 金額同士の比較を行う。
    *
    * 相対的に量が小さい方を「小さい」と判断する。通貨単位が異なる場合は [[java.lang.ClassCastException]] を スローするが、どちらか片方の量が`0`である場合は例外をスローしない。
    *
    * 例えば`10 USD`と`0 JPY`は、後者の方が小さい。 また、`0 USD`と`0 JPY`は同じである。
    *
    * @param that
    *   比較対象
    * @return
  ...
  /** このインスタンがあらわす金額が、負の金額かどうか調べる。
    ...
    *
    * @return
    *   負の金額である場合は`true`、そうでない場合は`false`
    */
  ...
  /** このインスタンがあらわす金額が、正の金額かどうか調べる。
    ...
    *
    * @return
    *   正の金額である場合は`true`、そうでない場合は`false`
    */
  ...
  /** この金額に`factor`を掛けた金額を返す。
    ...
    * 丸めモードは `RoundingMode#HALF_EVEN` を適用する。
    *
    * TODO: Many apps require carrying extra precision in intermediate calculations. The use of Ratio is a beginning,
    * but need a comprehensive solution. Currently, an invariant of Money is that the scale is the currencies standard
    * scale, but this will probably have to be suspended or elaborated in intermediate calcs, or handled with defered
  ...
  /** 指定したロケールにおける、単位つきの金額表現の文字列を返す。
    *
    * @param localeOption
    *   ロケールの`Option`。`None`の場合は `Locale#getDefault()` を利用する。
    * @return
    *   金額の文字列表現
    */
  ...
  /** 最小の単位金額を返す。
    ...
    *   最小の単位金額
    */
  private lazy val minimumIncrement: Money = {
    val increment =
...
object Money {
  ...
  /** [[scala.Iterable]]に含む全ての金額の合計金額を返す。
    *
    * 合計金額の通貨単位は、 `monies`の要素の（共通した）通貨単位となるが、 `Collection`が空の場合は、現在のデフォルトロケールにおける通貨単位で、量が0のインスタンスを返す。
    *
    * @param monies
    *   金額の集合
    * @return
